...
> ℹ️ **Nota sobre Physical Pack (HU-004A) y Physical Unpack (HU-004B)**: Implementan primitives transaccionales internos (`_wms_pack_physical`, `_wms_unpack_physical`); su exposición como comandos RF/API públicos está diferida a la incorporación del wrapper de idempotencia con `command_id`.
