...
- **Consumidores Concretos (HU-004A, HU-004B, HU-004C, HU-004D)**:
  ...
  - `stock.package._wms_split_physical()`: Persiste 2 eventos (`UNPACK` en fuente y `PACK` en destino) y el outbox `inventory.hu.split` (v1) de forma atómica junto a la mutación de `stock.move` y `stock.quant`.
  - `stock.package._wms_merge_physical()`: Persiste 2N eventos (`UNPACK` en fuente y `PACK` en destino por cada quant) y el outbox `inventory.hu.merged` (v1) de forma atómica junto a la mutación en batch de `stock.move` y `stock.quant`.
